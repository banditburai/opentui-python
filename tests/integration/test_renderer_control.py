"""Port of upstream renderer.control.test.ts plus Python-specific CliRenderer tests.

Upstream: packages/core/src/tests/renderer.control.test.ts
...
def _caps_with_osc52(enabled: bool = True) -> dict:
    """Return a minimal capabilities dict with osc52 set."""
    return {
...
class TestCliRendererClipboard:
    def test_copy_to_clipboard(self):
        r, n = _make()
        n.renderer._set_return("get_terminal_capabilities", _caps_with_osc52(True))
...
class TestBufferGetPlainText:
    async def test_returns_text(self):
        """Uses real native buffer to verify ctypes-based text extraction."""
        from opentui import test_render, Text

        setup = await test_render(lambda: Text("Hello"), {"width": 10, "height": 3})
        setup.render_frame()
        frame = setup.capture_char_frame()
        assert "Hello" in frame
        setup.destroy()

    def test_returns_empty_on_none_ptr(self):
        native_buf = _FakeBufferNS()
        native_buf._set_return("get_buffer_width", 10)
        native_buf._set_return("get_buffer_height", 5)
        native_buf._set_return("buffer_get_char_ptr", None)
        buf = Buffer(1, native_buf)
        assert buf.get_plain_text() == ""

    def test_returns_empty_on_exception(self):
        class _Exploding:
            def get_buffer_width(self, *a):
                raise RuntimeError("boom")

            def get_buffer_height(self, *a):
                raise RuntimeError("boom")
