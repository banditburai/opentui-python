...
# ANSI escape stripping for paste text (strips ANSI escape sequences)
_ANSI_RE = re.compile(r"\x1b\[[0-9;]*[A-Za-z]|\x1b\].*?(?:\x1b\\|\x07)|\x1b[^[\]()]")

# Kitty CSI-u: \x1b[<codepoint>[:<shifted>][:<base>][;[<modifiers>][:<event>]][;text]u
_KITTY_KEY_RE = re.compile(r"^\x1b\[(\d+(?::\d+)*)(?:;(\d+(?::\d*)*))?(?:;([\d:]+))?u$")

# Kitty functional key range
...
class InternalKeyHandler(KeyHandler):
    ...
    def on_internal(self, event: str, handler: Callable) -> None:
        """Register an internal (renderable) handler for an event."""
